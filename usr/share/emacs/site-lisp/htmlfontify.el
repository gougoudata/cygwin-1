...
(defcustom hfy-optimisations (list)
...
merged by the user - `hfy-flatten-style' should do this."
  ...
  (if fn
	...
	(if (eq key :inherit)
	    (setq parent (hfy-face-to-style-i
			  (hfy-face-attr-for-class val hfy-display-class))))
...
list of face properties instead of a face symbol"
  ...
  (let ((face-name   (hfy-p-to-face (text-properties-at p)))
        (overlay-data nil)
        (prop-seen    nil)
	(found-face   nil)
        (extra-props  nil))
...
(defun hfy-compile-face-map ()
  ...
  (let ((pt (point-min))
	...
	(if (setq fn (hfy-face-at pt))
	    (progn (if prev-tag (setq map (cons (cons pt 'end) map)))
	           (setq map (cons (cons pt fn) map))
		   (setq prev-tag t))
	  (if prev-tag (setq map (cons (cons pt 'end) map)))
	  (setq prev-tag nil))
	(setq pt (next-char-property-change pt)))
...
(defun hfy-fontify-buffer (&optional srcdir file)
  ...
  (let ((in-style                                   nil)
    ...
    (mapcar
	 ...
	 (if (eq 'end fn)
	     (if (not (and srcdir file)) (insert "</span>")
	       (insert "</span>")
